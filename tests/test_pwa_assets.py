...
def test_manifest_uses_real_pwa_icons_from_assets_folder():
    ...
    assert manifest_path.exists(), "El manifiesto PWA debe existir en la carpeta de iconos"
    ...
    for icon in manifest["icons"]:
        assert icon["src"].startswith("/iconos_sgtg_premium/"), (
            f"La ruta del icono debe apuntar a la carpeta de assets: {icon['src']}"
        )
        asset_path = project_root / "assets" / "iconos_sgtg_premium" / Path(icon["src"].split("/", 3)[-1])
