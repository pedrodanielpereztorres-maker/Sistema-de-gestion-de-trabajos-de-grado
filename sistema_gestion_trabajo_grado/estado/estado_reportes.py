        ...
        try:
            ...
            for row_num, t in enumerate(boveda.lista_trabajos_de_grado, 5):
                ...
                for col_num, valor in enumerate(datos_fila, 1):
                    ...
                    celda.alignment = (
                        alineacion_izq if col_num > 2 else alineacion_centro
                    )
                    celda.border = borde_delgado
                    if row_num % 2 == 0:
            ...
            try:
                with conn:
                    with conn.cursor() as cursor:
                        # 1. Resumen Global
                        cursor.execute(
                            "SELECT COUNT(*) FROM estudiante WHERE esta_activo = TRUE"
                        )
                        total = cursor.fetchone()[0]

                        cursor.execute(
                            "SELECT COUNT(*) FROM estudiante WHERE tutor_academico_id IS NOT NULL AND esta_activo = TRUE"
                        )
                        ...
                        max_est = (
                            max([r[1] for r in resultados_carreras])
                            if resultados_carreras
                            else 1
                        )
        ...
        try:
            ...
            for row_num, emp in enumerate(self.mejores_empresas, 5):
                ...
                datos_fila = [
                    emp["nombre"],
                    emp.get("direccion", ""),
                    emp.get("tutor", "No asignado"),
                    emp.get("correo") or "N/A",
                    emp.get("telefono") or "N/A",
                    emp["cantidad"],
                ]
                for col_num, valor in enumerate(datos_fila, 1):
                    celda = ws.cell(row=row_num, column=col_num, value=valor)
                    celda.font = fuente_normal
                    celda.alignment = (
                        alineacion_centro if col_num == 6 else alineacion_izq
                    )
                    celda.border = borde_delgado
                    if row_num % 2 == 0:
                        celda.fill = relleno_fila_par
    ...
    def exportar_empresas_pdf(self):
        ...
        try:
            ...
            pdf.cell(
                0,
                8,
                f"Total de Empresas Registradas: {len(self.mejores_empresas)}",
                ln=True,
            )
            pdf.ln(10)
            ...
            for emp in self.mejores_empresas:
                ...
                nombre = (
                    (emp["nombre"][:28] + "..")
                    if len(emp["nombre"]) > 30
                    else emp["nombre"]
                )
                direccion = (
                    (emp.get("direccion", "")[:32] + "..")
                    if len(emp.get("direccion", "")) > 34
                    else emp.get("direccion", "")
                )
                tutor_val = emp.get("tutor") or "No asignado"
                tutor_val = (
                    (tutor_val[:22] + "..") if len(tutor_val) > 24 else tutor_val
                )
                correo_val = emp.get("correo") or "N/A"
                correo_val = (
                    (correo_val[:32] + "..") if len(correo_val) > 34 else correo_val
                )
            ...
            return rx.download(
                data=pdf_output,
                filename=f"reporte_empresas_{datetime.now().strftime('%d_%m_%Y')}.pdf",
            )
        except Exception as e:
    ...
    async def exportar_trabajos_de_grado_pdf(self):
        ...
        try:
            ...
            for t in boveda.lista_trabajos_de_grado:
                ...
                tutor_acad_t = (
                    (tutor_acad_t[:22] + "..")
                    if len(tutor_acad_t) > 24
                    else tutor_acad_t
                )
                tutor_emp_t = t.get("tutor_empresa", "") or ""
                tutor_emp_t = (
                    (tutor_emp_t[:22] + "..") if len(tutor_emp_t) > 24 else tutor_emp_t
                )
                empresa_t = t.get("nombre_empresa", "") or ""
                empresa_t = (
                    (empresa_t[:22] + "..") if len(empresa_t) > 24 else empresa_t
                )
                ...
                pdf.cell(
                    25, 7, t.get("apellido_estudiante", "")[:15], 1, 0, "L", relleno
                )
