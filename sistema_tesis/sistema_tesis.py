...
app = rx.App(
    ...
    head_components=[
        ...
        rx.el.meta(name="apple-mobile-web-app-title", content="SGTG"),
        ...
        rx.el.link(rel="apple-touch-icon", href="/iconos_sgtg_premium/apple_touch_180x180.png"),
        rx.el.script("""
...
app.add_page(pagina_inicio, route="/", on_load=[
...
app.add_page(pagina_boveda, route="/boveda", on_load=[EstadoAutenticacion.verificar_sesion, EstadoBoveda.cargar_tesis])
app.add_page(pagina_estudiantes, route="/estudiantes", on_load=[EstadoAutenticacion.verificar_sesion_admin, EstadoEstudiante.cargar_estudiantes])
app.add_page(pagina_documentacion, route="/documentacion", on_load=[EstadoAutenticacion.verificar_sesion, EstadoDocumento.cargar_documentos])
...
app.add_page(pagina_perfil, route="/perfil", on_load=[EstadoAutenticacion.verificar_sesion, EstadoPerfil.cargar_datos_iniciales])
...
def servir_archivo_privado(request):
    ...
    token = request.query_params.get("token", "") or request.cookies.get("sts_token", "")

    if not verificar_token_acceso(token):
        return PlainTextResponse("Acceso Denegado: Sesión inválida o expirada.", status_code=401)

    if categoria not in ["trabajo_de_grado", "documentos"]:
        return PlainTextResponse("Acceso Denegado: Categoría no permitida.", status_code=403)
    ...
    return FileResponse(ruta_archivo, media_type="application/pdf", content_disposition_type="inline")

def servir_manifest(request):
    """Sirve el manifiesto PWA desde la carpeta de iconos del branding."""
    return FileResponse(os.path.join(PWA_ASSETS_DIR, "manifest.json"), media_type="application/manifest+json")


def servir_service_worker(request):
    """Sirve el service worker para habilitar la experiencia PWA."""
    return FileResponse(os.path.join(PWA_ASSETS_DIR, "sw.js"), media_type="application/javascript")


# Agregamos las rutas directamente al servidor ASGI interno de Reflex
try:
    app._api.mount("/iconos_sgtg_premium", StaticFiles(directory=PWA_ASSETS_DIR), name="iconos_sgtg_premium")
    app._api.add_route("/almacen/{categoria}/{archivo:path}", servir_archivo_privado, methods=["GET"])
