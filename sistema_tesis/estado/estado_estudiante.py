...
class EstadoEstudiante(rx.State):
    ...
    async def cargar_estudiantes(self) -> None:
        ...
        condiciones = ["e.esta_activo = TRUE"]
        params: list = []
        ...
        if self.filtro_estado == "En Pasantía":
            condiciones.append("e.tutor_academico_id IS NOT NULL")
        elif self.filtro_estado == "Sin Pasantía":
            condiciones.append("e.tutor_academico_id IS NULL")
            ...
            try:
                with conn2:
                    with conn2.cursor() as cursor:
                        cursor.execute(
                            f"SELECT COUNT(*) FROM estudiante e LEFT JOIN usuario u ON e.usuario_id = u.id LEFT JOIN carrera c ON e.carrera_id = c.id WHERE {where_clause}",
                            tuple(params),
                        )
        ...
        def _fetch_estudiantes(where_clause, params, limit, offset):
            conn2 = obtener_conexion()
            if conn2 is None:
                return []
            try:
                with conn2:
                    with conn2.cursor() as cur:
                        cur.execute(consulta, tuple(params) + (limit, offset))
                        return cur.fetchall()
            except Exception as exc:
            ...
            try:
                with conn3:
                    with conn3.cursor() as cur:
                        cur.execute("SELECT nombre FROM carrera WHERE esta_activa = TRUE ORDER BY nombre")
        ...
        try:
            ...
            rows = await asyncio.to_thread(_fetch_estudiantes, where_clause, params, self.registros_por_pagina, offset)
    ...
    def estudiantes_filtrados(self) -> List[Dict[str, Any]]:
        ...
        if self.busqueda_dinamica:
            ...
            lista = [e for e in lista if search in getattr(e, "cedula", "").lower() or search in getattr(e, "nombre", "").lower() or search in getattr(e, "apellido", "").lower()]
        if self.filtro_carrera and self.filtro_carrera != "Todas las carreras" and self.filtro_carrera != "":
            lista = [e for e in lista if getattr(e, "carrera", "") == self.filtro_carrera]
        if self.filtro_estado == "En Pasantía":
    ...
    async def abrir_modal_edicion_estudiante(self, cedula: str) -> None:
        ...
        est = next((e for e in self.lista_estudiantes if getattr(e, "cedula", "") == cedula_str), None)
        if not est:
            return
    ...
    async def cargar_datos_usuario(self) -> None:
        ...
        def _fetch_usuario_y_estudiante(cedula: str):
            ...
            try:
                ...
                logger.exception("Error al obtener datos de usuario/estudiante: %s", exc)
                return None, None
            finally:
                try:
                    conn.close()
        ...
        usuario_row, estudiante_row = await asyncio.to_thread(_fetch_usuario_y_estudiante, ced)
        if usuario_row:
        ...
        if estudiante_row:
            ...
            self.periodo_inicio = str(estudiante_row[2]) if estudiante_row[2] else self.periodo_inicio
            self.periodo_cierre = str(estudiante_row[3]) if estudiante_row[3] else self.periodo_cierre
    ...
    async def cargar_tutores_por_carrera(self, carrera: str = None) -> None:
        ...
        def _fetch_tutores(carrera: str):
            ...
            try:
                with conn:
                    with conn.cursor() as cursor:
                        cursor.execute("SELECT ta.id, ta.nombre || ' ' || ta.apellido FROM tutor_academico ta JOIN carrera c ON ta.carrera_id = c.id WHERE c.nombre = %s AND ta.esta_activo = TRUE ORDER BY ta.nombre", (carrera,))
    ...
    async def confirmar_eliminacion_estudiante(self) -> rx.Component:
        ...
        def _deactivate_student(usuario_id: int, cedula: str, password: str):
            ...
            try:
                with conn:
                    with conn.cursor() as cursor:
                        cursor.execute("SELECT contrasena_hash FROM usuario WHERE id = %s AND esta_activo = TRUE", (usuario_id,))
                        ...
                        cursor.execute("UPDATE estudiante SET esta_activo = FALSE WHERE cedula = %s", (cedula,))
                        cursor.execute("UPDATE usuario SET esta_activo = FALSE WHERE cedula = %s", (cedula,))
                    conn.commit()
                return True, ""
            except Exception as exc:
                try:
            ...
            try:
                with conn:
                    with conn.cursor() as cursor:
                        ...
                        if correo_in_use:
                            return False, "El correo ya está registrado para otro usuario."
    ...
    def lista_con_pasantia(self) -> List[Dict[str, Any]]:
        lista = [e for e in (self.lista_estudiantes or []) if getattr(e, "nombre_tutor", "")]
        return [{
            "cedula": getattr(e, "cedula", ""),
            "nombre": getattr(e, "nombre", ""),
            "apellido": getattr(e, "apellido", ""),
    ...
    def lista_sin_pasantia(self) -> List[Dict[str, Any]]:
        lista = [e for e in (self.lista_estudiantes or []) if not getattr(e, "nombre_tutor", "")]
        return [{
            "cedula": getattr(e, "cedula", ""),
            "nombre": getattr(e, "nombre", ""),
            "apellido": getattr(e, "apellido", ""),
    ...
    async def generar_reporte_estudiantes(self) -> rx.Component:
        ...
        try:
            ...
            headers = ["Cédula", "Nombre", "Apellido", "Carrera", "Teléfono", "Inicio", "Cierre", "Tutor Académico", "Empresa"]
            ...
            for row_num, e in enumerate(self.lista_estudiantes, 5):
                ...
                datos_fila = [
                    getattr(e, "cedula", ""), getattr(e, "nombre", ""), getattr(e, "apellido", ""),
                    getattr(e, "carrera", ""), getattr(e, "telefono_personal", ""),
                    getattr(e, "periodo_inicio", ""), getattr(e, "periodo_cierre", ""),
                    getattr(e, "nombre_tutor", ""), getattr(e, "nombre_empresa", "")
                ]
                for col_num, valor in enumerate(datos_fila, 1):
                    celda = ws.cell(row=row_num, column=col_num, value=valor)
                    celda.font = fuente_normal
                    celda.alignment = alineacion_izq if col_num in (2, 3, 4, 8, 9) else alineacion_centro
                    celda.border = borde_delgado
                    if row_num % 2 == 0:
    ...
    def exportar_estudiantes_pdf(self):
        ...
        try:
            ...
            for e in self.lista_estudiantes:
                ...
                empresa_t = (empresa_t[:20] + '..') if len(empresa_t) > 22 else empresa_t
                ...
                pdf.cell(23, 7, e.get("fecha_inicio_formateada", ""),    1, 0, 'C', relleno)
                pdf.cell(23, 7, e.get("fecha_cierre_formateada", ""),    1, 0, 'C', relleno)
